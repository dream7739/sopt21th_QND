...
import kotlinx.android.synthetic.main.survey_board_recycler.*
import org.sopt21.qnd.Adapter.CommentAdapter
import org.sopt21.qnd.Adapter.SurveyAdapter
...
class FragmentSurvey : Fragment(), View.OnClickListener {
    var checkReport: Int = 0
    val TAG: String = "LOG::SurveyFragment"

    private var networkService: NetworkService? = null
    private var surveyAdapter: SurveyAdapter? = null //리사이클러뷰 연결
    private var surveyList: ArrayList<SurveyResult.SurveyItemResult>? = null //창에 띄울 내용들을 담음
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        val surveyView: View = inflater!!.inflate(R.layout.survey_board_recycler, container, false) //borard_recyclerview 포함 view
        var main_list: RecyclerView = surveyView.findViewById(R.id.main_list) as RecyclerView //리사이클러뷰 xml과 연결

        /*surveyAdapter = SurveyAdapter(ArrayList<SurveyResult.SurveyItemResult>())
        surveyAdapter!!.setOnItemClickListener(this@FragmentSurvey)
        main_list.adapter = surveyAdapter
*/
        getSurveyList(user_id) //리스트에 담을 내용을 서버에서 가져오기
    ...
    //서버에서 리스트 가져오기--성공
    fun getSurveyList(token: String) {

        val surveyResponse = networkService!!.getSurveyList(token)

        surveyResponse.enqueue(object : Callback<SurveyResult> {
            override fun onResponse(call: Call<SurveyResult>?, response: Response<SurveyResult>?) {
                if (response!!.isSuccessful) {
                    Log.v(TAG, "success")
                    if (response!!.body().status.equals("Success")) {
                        surveyList = response!!.body().data
                        surveyAdapter = SurveyAdapter(response.body().data)
                        surveyAdapter!!.setOnItemClickListener(this@FragmentSurvey)
                        if(main_list != null) main_list.adapter = surveyAdapter
                    }
                } else {
                    Log.v(TAG, "fail")
            ...
            override fun onFailure(call: Call<SurveyResult>?, t: Throwable?) {
                    ...
                    if (response!!.body().status.equals("Success")) {
                        ...
                        comment_finish.setOnClickListener {
                            ...
                            if (comment_write.text.isEmpty()) {
                                ...
                                comment_dialog!!.dismiss() // 따라서 지워줌
                            }
                        }
            ...
            override fun onResponse(call: Call<SurveyAnswerResult>?, response: Response<SurveyAnswerResult>?) {
                if (response!!.isSuccessful) {
                    ...
                    if (answerList.servay_type == 0) //A타입
                    {
                        ...
                        s1_finish.setOnClickListener {
                            ...
                            notice_close.setOnClickListener {
                                ...
                                getSurveyList(user_id) //창끄면 설문 참여했는지 안했는지 반영해서 다시 띄워주기
                            }
                    ...
                    if (answerList.servay_type == 1) {
                        ...
                        requestManager = Glide.with(activity)
                        requestManager!!.load(answerList.servay_a_img).into(s2_option1)
                        requestManager!!.load(answerList.servay_b_img).into(s2_option2)


                        s2_option1.setOnClickListener {
                            s2_option1_check.setImageResource(R.drawable.btype_circle2)
                            s2_option2_check.setImageResource(R.drawable.btype_circle1)
                        ...
                        s2_option2.setOnClickListener {
                            s2_option2_check.setImageResource(R.drawable.btype_circle2)
                            s2_option1_check.setImageResource(R.drawable.btype_circle1)
                        ...
                        s2_finish.setOnClickListener {
                            ...
                            notice_close.setOnClickListener {
                                notice_dialog!!.dismiss() //노티스 창도 끄고
                                search_dialog2!!.dismiss() //참여 창도 끄고
                                getSurveyList(user_id) //창끄면 설문 참여했는지 안했는지 반영해서 다시 띄워주기
                            }
                    ...
                    if (answerList.servay_type == 2)  //C타입
                    {
                        ...
                        s3_finish.setOnClickListener {
                            ...
                            notice_close.setOnClickListener {
                                //X버튼
                                notice_dialog!!.dismiss() //노티스 창도 끄고
                                search_dialog3!!.dismiss() //참여 창도 끄고
                                getSurveyList(user_id) //창끄면 설문 참여했는지 안했는지 반영해서 다시 띄워주기
                            }
    ...
    override fun onClick(v: View?) {
        ...
        val idx: Int = main_list.getChildAdapterPosition(v)

        var user_id = user_id //토큰 값
        var servay_id = surveyList!!.get(idx).servay_id
        var surveyCoin = surveyList!!.get(idx).servay_get

        var answerCheck = surveyList!!.get(idx).servay_selected //응답했는지 안했는지

        if (answerCheck == 0) {//응답 안했으면 설문 참여 창으로
...
/*
 ...
 val resultView1=LayoutInflater.from(context).inflate(R.layout.survey_result_a,null) //설문 결과 type A
 val resultView2=LayoutInflater.from(context).inflate(R.layout.survey_result_b,null) //설문 결과 type B
 val resultView3=LayoutInflater.from(context).inflate(R.layout.survey_result_c,null) //설문 결과 type C
 val commentView=LayoutInflater.from(context).inflate(R.layout.comment_load,null) //댓글


 //띄울 dialog들

 var search_dialog2:AlertDialog?=null //설문 dialog type B
 var search_dialog3:AlertDialog?=null //설문 dialog type C


 var result_dialog2:AlertDialog?=null //결과 dialog type B
 var result_dialog3:AlertDialog?=null //결과 dialog type C
 var builder : AlertDialog.Builder = AlertDialog.Builder(activity) //dialog 띄울 builder
 var comment_dialog:AlertDialog?=null //댓글 dialog
 ...
 /****
 /*comment recyclerview 생성*/
 commentData.add(CommentData("하이","너무나 유익하네요!")) //서버에서 오는 데이터로 대체 (더미임)
 commentData.add(CommentData("하이","너무나 유익하네요!")) //서버에서 오는 데이터로 대체 (더미임)


 commentAdapter = CommentAdapter(commentData)
 commentList=commentView.findViewById(R.id.comment_list) as RecyclerView
 nManager= LinearLayoutManager(activity) //레이아웃 매니저
 commentList!!.layoutManager = nManager
 commentList!!.adapter = commentAdapter*****/


 /*******************dialog에 있는 각각의 view들을 정의*************/

 /**************************설문 결과*****************************/
 /* 설문 결과 dialog 뷰(dialogView) type A(그래프) */



 /* 설문 결과 dialog 뷰(dialogView) type B */
 var r2_good=resultView2.findViewById(R.id.r2_btn_good)
 var r2_comment=resultView2.findViewById(R.id.r2_btn_comment)
 var r2_close=resultView2.findViewById(R.id.r2_btn_close)
 var r2_text=resultView2.findViewById(R.id.r2_text_main)
 var r2_image1=resultView2.findViewById(R.id.r2_image_option1)
 var r2_image2=resultView2.findViewById(R.id.r2_image_option2)
 var r2_text_ok=resultView2.findViewById(R.id.r2_text_ok)
 var r2_text_no=resultView2.findViewById(R.id.r2_text_no)
 var r2_text_result1=resultView2.findViewById(R.id.r2_text_result1)
 var r2_text_result2=resultView2.findViewById(R.id.r2_text_result2)


 /* 설문 결과 dialog 뷰(dialogView) type C */
 var r3_good=resultView3.findViewById(R.id.r3_btn_good)
 var r3_comment=resultView3.findViewById(R.id.r3_btn_comment)
 var r3_close=resultView3.findViewById(R.id.r3_btn_close)
 var r3_text=resultView3.findViewById(R.id.r3_text_main)
 var r3_image1=resultView3.findViewById(R.id.r3_image_option1)
 var r3_image2=resultView3.findViewById(R.id.r3_image_option2)
 var r3_text_ok=resultView3.findViewById(R.id.r3_text_ok)
 var r3_text_no=resultView3.findViewById(R.id.r3_text_no)
 var r3_text_result1=resultView3.findViewById(R.id.r3_text_result1)
 var r3_text_result2=resultView3.findViewById(R.id.r3_text_result2)


 /*댓글 dialog 뷰*/
 var comment_close=commentView.findViewById(R.id.comment_close)
 var comment_write=commentView.findViewById(R.id.comment_write)
 var comment_finish=commentView.findViewById(R.id.comment_finish)
 ...
 result_dialog2=builder.create()
 result_dialog2!!.setView(resultView2)


 result_dialog3=builder.create()
 result_dialog3!!.setView(resultView3)
...
/
 ...
 s2_declar.setOnClickListener{ //설문 시 신고 버튼(신고 다이얼로그 디쟌 안나옴)
     declar_dialog!!.show()
 ...
 s2_good.setOnClickListener{ //설문 시 좋아요 버튼

 }

 s2_close.setOnClickListener{ //설문 시 닫기 버튼
     search_dialog2!!.dismiss()}


 s2_finish.setOnClickListener{ //설문 시 완료 버튼
     search_dialog2!!.dismiss()
     notice_dialog!!.show()

     notice_show.setOnClickListener{ //노티스 다음 화면 보이기
         notice_dialog!!.dismiss() //노티스 다이얼로그 없애고
         result_dialog2!!.show() //설문 결과 창 띄우기


         /*설문 결과 event
         기능1: 좋아요 */
         r2_good.setOnClickListener {

         }

         /*설문 결과 event
         기능2: 댓글 */
         r2_comment.setOnClickListener {
             comment_dialog!!.show() //코멘트 창 띄우기

             comment_close.setOnClickListener {
                 comment_dialog!!.dismiss()
             }
             comment_finish.setOnClickListener {
                 //서버로 댓글 등록하고 위에 리사이클러뷰에 띄우기
             }


         }

         r2_close.setOnClickListener {
             result_dialog2!!.dismiss()
